...
function prepareFilters(filters, togglers) {
  ...
  for (let filter in filters) {
    if (filters[filter] && togglers[filter])
      preparedFilters = `${preparedFilters}&f_${filter}=${filters[filter]}`;
  }
...
async function getUFs(filters, togglers) {
  const { data } = await axios.get(
    `${import.meta.env.VITE_SERVER}commission/ufs?none=1${prepareFilters(filters, togglers)}`,
  );
...
export default function MapPP() {
  ...
  useEffect(() => {
    async function fetchData(page = 1, filters) {
      _loading(true);
      const { data } = await axios.get(
        `${import.meta.env.VITE_SERVER}commission/?limit=6&page=${page}${prepareFilters(filters, togglers)}`,
      );
      _loading(false);
  ...
  useEffect(() => {
    ...
    async function fetchGeoData() {
      ...
      const { data } = await axios.get(
        `${import.meta.env.VITE_SERVER}commission/geo/?${prepareFilters(filters, togglers)}`,
      );
          ...
          >
            <div className={`row ${styles.filters}`}>
              ...
              <div className={styles.each}>
                <div>
                  <Toggler
                    checked={togglers["regioes"]}
                    onToggle={(checked) => handleToggle("regioes")(checked)}
                  />
                </div>
                <div>Regiões</div>
                <div>
                    ...
                    <div>
                      <StyledReactSelect
                        ...
                        onChange={(selectedOption) =>
                          onFilterChange("regioes", selectedOption)
                        }
              ...
              <div className={styles.each}>
                <div>
                  <Toggler
                    checked={togglers["ufs"]}
                    onToggle={(checked) => handleToggle("ufs")(checked)}
                  />
                </div>
                <div>Estado</div>
                <div>
                  {ufs && (
                    <div>
                      <StyledReactSelect
                        ...
                        {...selectDefaults}
                        onChange={(selectedOption) =>
                          onFilterChange("ufs", selectedOption)
                        }
                        closeMenuOnSelect={false}
                        components={animatedComponents}
                        isMulti
