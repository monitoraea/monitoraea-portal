...
export default function Timeline({ portal="home", url="na-midia", title="MonitoraEA na mídia", staleTime = 3600000 /* 1h */ }) {
  const isMobile = useMediaQuery({ maxWidth: 991 });

  const { data } = useQuery([`timeline_${portal}`], {
    queryFn: async () =>
      (await axios.get(`${import.meta.env.VITE_SERVER}${url}`))
        .data,
    staleTime,
  });
          ...
          <div className={styles["width-limiter"]}>
            ...
            <div className={styles.timeline}>
              <div className={styles["timeline-each"]}>
                <div></div>
                <div
                  className={`${styles["timeline-each-thumb"]} ${styles.empty}`}
                ></div>
              </div>

              {data.list.map((nm) => (
                <div key={nm.id} className={styles["timeline-each"]}>
              ...
              {/*<div className={styles["timeline-each"]}>
                ...
                <div className={styles["timeline-each-more"]}>
                  <a href="">Mais</a>
                </div>
      ...
      {data && (
        <div className="width-limiter">
          ...
          <div className="timeline">
            ...
            {data.list.map((nm) => (
              <div key={nm.id} className="timeline-each">
